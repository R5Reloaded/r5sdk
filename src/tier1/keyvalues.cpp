...
bool KeyValues::WriteAsBinary(CUtlBuffer& buffer) const
{
	...
	for (const KeyValues* dat = this; dat != NULL; dat = dat->m_pPeer)
	{
		...
		case TYPE_WSTRING:
		{
			short nLength = dat->m_wsValue ? (short)wcslen(dat->m_wsValue) : 0;
		...
		case TYPE_VECTOR:
		{
			buffer.PutFloat(dat->m_Vector[0]);
			buffer.PutFloat(dat->m_Vector[1]);
			buffer.PutFloat(dat->m_Vector[2]);
			break;
		}
		case TYPE_PTR:
		{
...
bool KeyValues::ReadAsBinary(CUtlBuffer& buffer, int nStackDepth)
{
	...
	if (nStackDepth > 100)
	{
		Assert(false, "KeyValues::ReadAsBinary() stack depth > 100\n");
		return false;
	}

	KeyValues* dat = this;
	KeyValuesTypes_t type = (KeyValuesTypes_t)buffer.GetUnsignedChar();
	...
	while (true)
	{
		...
		case TYPE_VECTOR:
		{
			dat->m_Vector[0] = buffer.GetFloat();
			dat->m_Vector[1] = buffer.GetFloat();
			dat->m_Vector[2] = buffer.GetFloat();
			break;
		}
		case TYPE_PTR:
		{
		...
		type = (KeyValuesTypes_t)buffer.GetUnsignedChar();
...
bool KeyValues::ReadAsBinaryFiltered(CUtlBuffer& buffer, int nStackDepth)
{
	...
	if (nStackDepth > 100)
	{
		Assert(false, "KeyValues::ReadAsBinaryFiltered() stack depth > 100\n");
		return false;
	}
